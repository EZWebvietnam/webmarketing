...
class Advmodel extends CI_Model
{
    ...
    {
        $id = intval($id);
        $sql = "SELECT adv_detail.id as id_detail,adv_detail.file,adv_detail.exp_date,adv.name FROM adv_detail INNER JOIN adv ON adv.id = adv_detail.vi_tri WHERE adv_detail.id = $id";
        $query = $this->db->query($sql);
        return $query->result_array();
    }
}
